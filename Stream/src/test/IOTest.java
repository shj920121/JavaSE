...
public class IOTest {
	public String readline() {
		InputStreamReader isr = new InputStreamReader(System.in); // reader -> 16bit
		...
		try {
			do {
				...
				if (idx == buffer.length - 1)// 이때 배열에 내용이 가득차있는 상태
					buffer = reallocBuffer(buffer); // realloc 재할당 뜻
	...
	public void println(String msg) {
		...
		OutputStreamWriter osw = new OutputStreamWriter(System.out); // writer -> 16bit 지원
		try {
			osw.write(buffer);
			osw.flush();
		} catch (IOException e) {
